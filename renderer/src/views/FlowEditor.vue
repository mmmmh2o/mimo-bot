          ...
          <template v-if="selectedNode.type === 'send-message' && !nodeManifests['send-message']">
            ...
            <el-form-item label="发送模式">
              <el-select v-model="selectedNode.data.mode" style="width:100%">
                <el-option label="🔌 适配器模式（自动识别页面）" value="adapter" />
                <el-option label="🎯 自定义模式（指定选择器）" value="custom" />
              </el-select>
            </el-form-item>

            <!-- 自定义模式字段 -->
            <template v-if="selectedNode.data.mode === 'custom'">
              <el-divider content-position="left">输入设置</el-divider>
              <el-form-item label="输入目标（CSS 选择器）">
                <el-input v-model="selectedNode.data.inputSelector" placeholder="如 textarea, #msg-input" />
              </el-form-item>
              <el-form-item label="输入方式">
                <el-select v-model="selectedNode.data.inputMethod" style="width:100%">
                  <el-option label="逐字输入（模拟键盘）" value="type" />
                  <el-option label="粘贴填入（一次性）" value="fill" />
                </el-select>
              </el-form-item>

              <el-divider content-position="left">发送设置</el-divider>
              <el-form-item label="发送方式">
                <el-select v-model="selectedNode.data.sendMethod" style="width:100%">
                  <el-option label="点击按钮" value="click" />
                  <el-option label="按回车" value="enter" />
                  <el-option label="回车 + 点击" value="both" />
                </el-select>
              </el-form-item>
              <el-form-item label="发送按钮（CSS 选择器）" v-if="selectedNode.data.sendMethod !== 'enter'">
                <el-input v-model="selectedNode.data.sendSelector" placeholder="如 button.send, #submit" />
              </el-form-item>
            </template>
            ...
            <template v-if="selectedNode.data.waitForReply">
              <el-form-item label="回复容器（CSS 选择器）" v-if="selectedNode.data.mode === 'custom'">
                <el-input v-model="selectedNode.data.replySelector" placeholder="如 .reply, .message:last-child" />
              </el-form-item>
